...
class ApiKeyPopup(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.save_btn = ctk.CTkButton(self, text="Verify Credentials", fg_color=MAIN_BLUE, hover_color=HOVER_BLUE, height=45, corner_radius=CORNER_RADIUS, command=self.start_verification)
...
class CreativeAnalyzer(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.analyze_btn = ctk.CTkButton(self.sidebar, text="Run Analysis", state="disabled", fg_color="#10b981", hover_color="#059669", height=45, corner_radius=CORNER_RADIUS, command=self.start_analysis_thread)
        ...
        self.reset_btn = ctk.CTkLabel(self.sidebar, text="Reset API Key", font=ctk.CTkFont(size=12), text_color="gray", cursor="hand2")
        self.reset_btn.pack(side="bottom", pady=30)
        self.reset_btn.bind("<Enter>", lambda e: self.reset_btn.configure(font=ctk.CTkFont(size=12, underline=True)))
        self.reset_btn.bind("<Leave>", lambda e: self.reset_btn.configure(font=ctk.CTkFont(size=12, underline=False)))
        ...
        self.copy_btn = ctk.CTkButton(self.content_area, text="Copy Report", fg_color="transparent", border_width=1, border_color=MAIN_BLUE, text_color=MAIN_BLUE, hover_color="#eef6ff", corner_radius=CORNER_RADIUS, command=self.copy_to_clipboard)
    ...
    def start_analysis_thread(self):
        self.status_label.configure(text="ENGINEERING ANALYSIS IN PROGRESS...")
    ...
    def run_analysis(self):
        try:
            ...
            {"spelling": "accuracy feedback", "design": "layout feedback", "hierarchy": "visual flow feedback", "engagement": "suitability feedback", "suggestions": ["s1", "s2", "s3"]}"""
